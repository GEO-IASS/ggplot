function [ colors_index, the_colors, range_type ] = assign_colors( data, color_col, varargin )
...
% Update:
    ...
    if is_categorical(data{:, color_col})
        ...
        the_colors = linspecer(length(cats),'sequential');
        
        range_type = 'continuous';
    end
    
    for i = 1:length(cats)
        color_idx = mod(i, length(the_colors));
